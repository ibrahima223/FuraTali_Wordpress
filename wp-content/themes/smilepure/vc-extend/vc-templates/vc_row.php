...
if ( Smilepure::setting( 'lazy_load_images' ) && $atts['background_image'] !== '' ) {
	$_url                 = wp_get_attachment_image_url( $atts['background_image'], 'full' );
	$css_classes[]        = 'tm-lazy-load';
	$wrapper_attributes[] = 'data-src="' . esc_url( $_url ) . '"';
}
...
Smilepure_VC::get_vc_row_css( '#' . $el_id, $atts );
...
if ( $separator_type !== '' ) {
	...
	$output            .= '<div class="' . $separator_classes . '">' . Smilepure_VC::get_separator_svg( $separator_type ) . '</div>';
}
