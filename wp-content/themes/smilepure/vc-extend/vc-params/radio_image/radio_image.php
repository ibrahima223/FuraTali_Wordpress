...
*/
if ( ! class_exists( 'Smilepure_VC_Radio_Image' ) ) {
	class Smilepure_VC_Radio_Image {
		...
		public function render() {
			...
			foreach ( $values as $value => $label ) {
				...
				$output .= '<div ' . esc_html( $title ) . ' ><img src="' . esc_url( $label ) . '" /></div>';
				$output .= '</label>';
			}
			$output .= '</div>';
...
if ( class_exists( 'Smilepure_VC_Radio_Image' ) ) {
	...
	function Smilepure_vc_image_radio_settings_field( $settings, $value ) {

		$field = new Smilepure_VC_Radio_Image( $settings, $value );

		return $field->render();
	}

	WpbakeryShortcodeParams::addField( 'image_radio', 'Smilepure_vc_image_radio_settings_field', SMILEPURE_THEME_URI . '/vc-extend/vc-params/radio_image/scripts.js' );
}
