 ...
 * Example
	array(
			'type' => 'gradient',
			'class' => '',
			'heading' => esc_html__('Gradient Type', 'smilepure'),
			'param_name' => 'bg_grad',
			'description' => esc_html__('At least two color points should be selected.', 'smilepure'),
			'dependency' => array('element' => 'enable_overlay', 'value' => array('enable')),
			'group' => $group_name,
		)
*/
if ( ! class_exists( 'Smilepure_VC_Gradient_Param' ) ) {

	class Smilepure_VC_Gradient_Param {
		...
		function gradient_picker( $settings, $value ) {
			...
			$output .= '<select id="grad_type' . $uni . '" class="grad_type" data-uniqid="' . $uni . '">
				<option value="vertical">' . esc_html__( 'Vertical', 'smilepure' ) . '</option>
				<option value="horizontal">' . esc_html__( 'Horizontal', 'smilepure' ) . '</option>
				<option value="custom">' . esc_html__( 'Custom', 'smilepure' ) . '</option>
			</select>
			...
			$output .= '<div class="wpb_element_label" style="margin-top: 10px;">' . esc_html__( 'Choose Colors', 'smilepure' ) . '</div>';
	...
	new Smilepure_VC_Gradient_Param();
}
